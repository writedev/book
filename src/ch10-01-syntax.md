...
<Listing number="10-4" file-name="src/main.rs" caption="Deux fonctions qui diffèrent seulement par leurs noms et les types dans leurs signatures">
...
<Listing number="10-5" file-name="src/main.rs" caption="La fonction `largest` utilisant des paramètres de type génériques ; cela ne compile pas encore">
...
<Listing number="10-6" file-name="src/main.rs" caption="Une structure `Point<T>` qui contient des valeurs `x` et `y` de type `T`">
...
<Listing number="10-7" file-name="src/main.rs" caption="Les champs `x` et `y` doivent être du même type car les deux ont le même type de données générique `T`.">
...
<Listing number="10-8" file-name="src/main.rs" caption="Une `Point<T, U>` générique sur deux types afin que `x` et `y` puissent être des valeurs de types différents">
...
<Listing number="10-9" file-name="src/main.rs" caption="Implémentation d'une méthode nommée `x` sur la structure `Point<T>` qui renverra une référence au champ `x` de type `T`">
...
<Listing number="10-10" file-name="src/main.rs" caption="Un bloc `impl` qui ne s'applique qu'à une structure avec un type concret particulier pour le paramètre de type générique `T`">
...
<Listing number="10-11" file-name="src/main.rs" caption="Une méthode qui utilise des types génériques différents de ceux de la définition de sa structure">
